...
/*
// testAccCheckResourceAttrAnonymousRegionalARN ensures the Terraform state exactly matches a formatted ARN with region but without an account ID
func testAccCheckResourceAttrAnonymousRegionalARN(resourceName, attributeName, arnService, arnResource string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		attributeValue := arn.ARN{
			Partition: testAccGetPartition(),
			Region:    testAccGetRegion(),
